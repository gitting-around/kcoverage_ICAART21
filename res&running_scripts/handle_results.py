...
def static_cov(no_agents, no_targets, sim_duration, coverage, tg_update, rrmodel):
    ################################### ACTIVE COVERAGE ##########################################
    data = np.zeros((no_targets, sim_duration-1))
    data_k = np.zeros((no_targets, sim_duration-1))

    data_all = np.zeros((no_targets, sim_duration-1))
    #print data
    #for x in range(no_agents):
    for x in range(no_agents):
        with open("output_"+str(x+1), "r") as f:
    ...
    if tg_update == 0:
        ...
        staticdynamic = "static"
    case = str(no_agents) + "_" + str(no_targets) + "_" + str(sim_duration) + "_" + str(coverage) + "_" + staticdynamic + "_" + rrmodel
    ...
    datac = np.zeros((no_targets, sim_duration-1))
    datac_k = np.zeros((no_targets, sim_duration-1))
    datac_all = np.zeros((no_targets, sim_duration-1))
    #print datac
    #for x in range(no_agents):
    for x in range(no_agents):
        with open("output_"+str(x+1), "r") as f:
    ...
    #print datac
    #Compute avg(tt1c_vector) - Average time to get 1 agent covering a target
    ave_time, min_time_all_cov = computeMetrics(data=datac, coverage=1)

    print "1-Coverage ave time:\t\t%g" % ave_time
    print "1-Coverage min to cover all:\t%g" % min_time_all_cov

    ##K-COVERAGE######################################
    ave_time_k, min_time_all_cov_k = computeMetrics(data=datac_k, coverage=coverage)
    ...
    #WRITE TO csv
    if tg_update == 0:
        staticdynamic = "combined_dynamic"
    else:
        staticdynamic = "combined_static"
    case = str(no_agents) + "_" + str(no_targets) + "_" + str(sim_duration) + "_" + str(coverage) + "_" + staticdynamic + "_" + rrmodel
    fname = "/home/mfi01/catkin_ws/" + "_" + case + ".csv"
...
def dynamic_k_cov(no_agents, no_targets, sim_duration, coverage, tg_update, rrmodel):
    ############################## Active coverage ####################################
    data = np.zeros((no_targets, sim_duration-1))
    #print data
    for x in range(no_agents):
    ...
    print "ave time: %f" % ave_time
    #ave of agents covering a target
    agent_tg = np.average(datac, axis=1)
    print agent_tg
    ave_agent = np.average(agent_tg)
    print "ave agents: %f" % ave_agent
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 7:
        print "Usage: ./handle_results.py no_agents no_targets sim_duration coverage static/dynamic model_code"
    ...
    if int(sys.argv[5]) == 1:
        ...
        static_cov(int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]), sys.argv[6])
        #static_k_cov(int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]))
    else:
        print "im in dynamic mode"
        dynamic_k_cov(int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]), sys.argv[6])
