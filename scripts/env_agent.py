...
class Environment:
    def __init__(self, size, init_target_dist, no_targets):
        ...
        s = int(rospy.get_param('/seed'))*11 + 10
    ...
    def generate_space(self, no_targets):
        ...
        for i in range(no_targets):
            ...
            '''kuadrant = random.choice([1,2,3])
            if kuadrant==1:
                xpos = np.random.uniform(0, 50)
                ypos = np.random.uniform(50, 100)
            elif kuadrant==2:
                xpos = np.random.uniform(50, 100)
                ypos = np.random.uniform(50, 100)
            else:
                xpos = np.random.uniform(50, 100)
                ypos = np.random.uniform(0, 50)'''
            ...
            with open(self.base_path+"/tg_interest_"+str(-1), 'a+') as f:
                f.write(str(interest)+"\n")
            print "Tg %d with interest %f" % (id, interest)
    ...
    def callback_motion(self, data):
        ...
        if not found:
            ...
            noag = 0
            inserted = False
            for index, xx in enumerate(self.objects_in_space):
                if xx['type'] == 'agent':
                    noag +=1
                    if xx['id'] > data.id:
                        self.objects_in_space.insert(index, {'type': 'agent', 'id': data.id, 'xpos': data.xpos, 'ypos': data.ypos, 'velocity': data.velocity, 'direction': data.direction, 'change_dir': data.omega, 'interest': 0.0})
                        inserted = True
                        break
            if noag==0 or not inserted:
                self.objects_in_space.append({'type': 'agent', 'id': data.id, 'xpos': data.xpos, 'ypos': data.ypos, 'velocity': data.velocity, 'direction': data.direction, 'change_dir': data.omega, 'interest': 0.0})
            '''
            self.objects_in_space.append({'type': 'agent', 'id': data.id, 'xpos': data.xpos, 'ypos': data.ypos, 'velocity': data.velocity, 'direction': data.direction, 'change_dir': data.omega, 'interest': 0.0})
            '''
            self.lock.release()
